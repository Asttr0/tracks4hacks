...
function shareTechnique(attack, alert) {
    const atkT = new Set(attack.mitre ?? []);
    const alertT = new Set(alert.rule.mitre?.id ?? []);
    for (const t of atkT) {
        if (alertT.has(t))
            return true;
        for (const at of alertT)
            if (at.startsWith(t) || t.startsWith(at))
                return true;
    }
    return false;
}
export function correlate(attacks, alerts, opts = {}) {
    const windowMs = (opts.windowSec ?? 120) * 1000;
    const bands = [];
    for (const atk of attacks) {
        const atkTs = Date.parse(atk.timestamp);
        if (Number.isNaN(atkTs))
            continue;
        const matched = [];
        let maxLevel = 0;
        for (const al of alerts) {
            const alTs = Date.parse(al.timestamp);
            if (Number.isNaN(alTs))
                continue;
            const dt = alTs - atkTs;
            if (dt < -5_000 || dt > windowMs)
                continue;
            if (shareTechnique(atk, al)) {
                matched.push(al);
                if (al.rule.level > maxLevel)
                    maxLevel = al.rule.level;
            }
        }
        if (matched.length) {
